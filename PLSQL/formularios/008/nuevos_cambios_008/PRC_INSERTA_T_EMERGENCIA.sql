CREATE
OR REPLACE PROCEDURE PRC_INSERTA_T_EMERGENCIA (
    ...
    v_custodia_policial char,
    v_notificacion char,
    v_vivo varchar2,
    v_fallecido_egreso varchar2,
    v_estable_egreso varchar2,
    v_inestable_egreso varchar2,
    v_alta_definitiva varchar2,
    v_consulta varchar2,
    v_observacion_emergencia varchar2,
    v_hospitalizacion varchar2,
    v_referencia varchar2,
    v_referencia_inversa varchar2,
    v_derivacion varchar2,
    v_establecimiento varchar2,
    v_alta_voluntaria varchar2,
    v_retiro_sin_autorizacion varchar2,
    v_morgue varchar2,
    v_funeraria varchar2,
    v_medicina_legal varchar2,
    v_observaciones varchar2,
    v_dias_de_reposo varchar2

) IS V_CUERPO_HUMANO CLOB;
...
EXCEPTION WHEN OTHERS THEN V_CUERPO_HUMANO := NULL;
...
BEGIN
select
    max(a.cd_documento_clinico) INTO V_REGISTRO_EDITOR
from
    pw_editor_clinico a, -- enlace documento clinico-documento editor
    pw_documento_clinico b, -- documento clinico
    editor.editor_registro c
where
    a.cd_documento in (645)
    and a.cd_documento_clinico = b.cd_documento_clinico
    and b.tp_status = 'FECHADO'
    and a.cd_editor_registro = c.cd_registro
    and b.cd_atendimento = v_cd_atendimento;

EXCEPTION WHEN OTHERS THEN V_REGISTRO_EDITOR := NULL;
...
EXCEPTION WHEN OTHERS THEN V_SECUENCIAL_MAYOR := NULL;

END;

INSERT INTO
    EDITOR_CUSTOM.T_EMERGENCIA (
        CD_FORMULARIO,
        CD_ATENDIMENTO,
        FECHA_REGISTRO,
        FUENTE_INFORMACION,
...
VALUES
    (
        ...
        v_alta_definitiva,
        v_consulta,
        v_observacion_emergencia,
        v_hospitalizacion,
        v_referencia,
        v_referencia_inversa,
        v_derivacion,
        v_establecimiento,
        v_alta_voluntaria,
        v_retiro_sin_autorizacion,
        v_morgue,
        v_funeraria,
        v_medicina_legal,
        v_observaciones,

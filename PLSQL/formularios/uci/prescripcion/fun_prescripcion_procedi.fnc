create or replace function fun_prescripcion_procedi (vAtendimento number) return clob is  clob;

begin
SELECT
  upper(
    t7.ds_tip_presc || ' ' || t4.qt_infusao || ' ' || decode(
      trim(t10.ds_for_apl),
      'NO APLICA',
      ' ',
      t10.ds_for_apl
      ...
      ' ',
      t9.ds_tip_fre
    ) || ' ' || fun_detalle_componentes (t4.cd_itpre_med) || ' ' || t4.ds_itpre_med
  ) prescripcion_pme INTO vResultado
from
  pre_med t1,
  itpre_med t4,
  tip_presc t7,

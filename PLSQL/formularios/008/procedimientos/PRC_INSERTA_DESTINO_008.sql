...
-- ESCOGE EL DESTINO
if trim(vDestino1) = 'true' then vDestino := 'Domicilio';

elsif trim(vDestino2) = 'true' then vDestino := 'Consulta externa';

elsif trim(vDestino3) = 'true' then vDestino := 'Observación';

elsif trim(vDestino4) = 'true' then vDestino := 'Referencia';

elsif trim(vDestino5) = 'true' then vDestino := 'Abandono Voluntario';

elsif trim(vDestino6) = 'true' then vDestino := 'Abandono sin autorización';

elsif trim(vDestino7) = 'true' then vDestino := 'Morgue';

elsif trim(vDestino8) = 'true' then vDestino := 'Funeraria';

elsif trim(vDestino9) = 'true' then vDestino := 'Medicina Legal';

elsif trim(vDestino10) = 'true' then vDestino := 'Internación';
